...
export function DayHabit({ amount=0, completed=0, date, ...rest }: DayHabitProps) {
    ...
    return (
        <TouchableOpacity
            className={clsx('rounded-lg border-2 m-1 border-violet-600', {
                ['bg-zinc-900 border-zinc-800']: progressPercentage === 0,
                ['bg-violet-100']: progressPercentage > 0 && progressPercentage < 20,
                ['bg-violet-300']: progressPercentage >= 20 && progressPercentage < 40,
                ['bg-violet-500']: progressPercentage >= 40 && progressPercentage < 60,
                ['bg-violet-700']: progressPercentage >= 60 && progressPercentage < 80,
                ['bg-violet-900']: progressPercentage >= 80,
                ['border-yellow-300']: isToday
            })}
